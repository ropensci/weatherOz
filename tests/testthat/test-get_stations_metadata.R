...
test_that("get_stations_metadata() matches station_codes", {
  ...
  expect_identical(as.character(x$station_code), c("YU001", "YU002", "YU003"))
})

# no api_key
test_that("get_stations_metadata() errors if no api_key is provide for DPIRD", {
  expect_error(get_stations_metadata(which_api = "dpird", api_key = ""))
  expect_error(get_stations_metadata(which_api = "all", api_key = ""))
})
