...
.query_silo_api <- function(.station_code,
  ...
  response <-
    ...
    httr2::req_url_query(!!!silo_query_list) |>
    httr2::req_perform()
    ...
    if (grepl("Sorry", httr2::resp_body_string(resp = response))) {
      stop(call. = FALSE,
           gettextf(httr2::resp_body_string(resp = response)),
           domain = NA)
    }

    data.table::fread(httr2::resp_body_string(resp = response))
